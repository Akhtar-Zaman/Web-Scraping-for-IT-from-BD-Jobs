...
url  = "https://jobs.bdjobs.com/jobdetails.asp?id=872656&fcatId=8&ln=1"
...
file = open('Record21.csv', 'w')
################################################################################################################
writer = csv.writer(file)
writer.writerow(['JOB TITLE','COMPANY NAME','VACANCY','EMPLOYMENT STATUS', 'EXPERIENCE','GENDER','AGE','JOB LOCATION','JOB RESPONSIBILITIES','EDUCATIONAL REQUIREMENTS','ADDITIONAL REQUIREMENTS','SALARY','COMPENSATION & AND OTHER BENEFITS','PUBLISHED ON','APPLICATION DEADLINE'])
...
for item in containers:
    ...
    Job_Responsibilities = item.find("div", {"class": "job_des"})
    job = Job_Responsibilities.ul.text

    Educational_Requirements = item.find("div", {"class": "edu_req"})
    edu_req = Educational_Requirements.ul.text

    Additional_req = item.find("div", {"class": "job_req"})
    add_req = Additional_req.ul.text
...
##########################################################
    ...
    Application_Deadline = h4_tags[8].text
    app_dead = Application_Deadline.replace("Application Deadline:", "")"""
########################################################################################

    Job_Location = h4_tags[4].text
    joblctn = Job_Location.replace("Job Location:", "")

    #sal = ""
    Salary = h4_tags[5].text
    sal = Salary.replace("Salary:", "")

    Application_Deadline = h4_tags[6].text
    app_dead = Application_Deadline.replace("Application Deadline:", "")


    writer.writerow([ job_titie, company_name, vac, employment_status, exp, gen, ag, joblctn, job, edu_req, add_req, sal, comp, pub_on, app_dead])
